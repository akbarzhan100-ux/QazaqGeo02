...
const getImageUrl = (id: string, width: number, height: number) => {
  const imageMap: Record<string, string> = {
    'astana': 'https://upload.wikimedia.org/wikipedia/commons/c/ce/Astana_DSC04362_%287711355642%29.jpg',
    'almaty-city': 'https://upload.wikimedia.org/wikipedia/commons/8/8c/Almaty_city_skyline.jpg',
    'shymkent': 'https://upload.wikimedia.org/wikipedia/commons/4/41/Tulip-Fountain-Shymkent-Kazakhstan.jpg',
    'abai': '1500382017468-9049fed747ef',
    'akmola': '1518098268026-4e89f1a2cd8e',
    'aktobe': '1542601098-8fc114e148e2',
    'almaty': '1518098268026-4e89f1a2cd8e',
    'atyrau': 'https://upload.wikimedia.org/wikipedia/commons/6/68/%D0%A3%D1%80%D0%B0%D0%BB_%D1%80%D0%B5%D0%BA%D0%B0.jpg',
    'east-kazakhstan': '1465146344425-f00d5f5c8f07',
    'jambyl': '1542601098-8fc114e148e2',
    'jetisu': 'https://upload.wikimedia.org/wikipedia/commons/3/38/Altynemel_landscape.jpg',
    'karaganda': '1500382017468-9049fed747ef',
    'kostanay': '1500382017468-9049fed747ef',
    'kyzylorda': 'https://upload.wikimedia.org/wikipedia/commons/1/14/Baikonuriss.jpg',
    'mangystau': 'https://upload.wikimedia.org/wikipedia/commons/d/dc/%D0%91%D0%BE%D1%81%D0%B6%D0%B8%D1%80%D0%B07.jpg',
    'north-kazakhstan': '1542601098-8fc114e148e2',
    'pavlodar': 'https://upload.wikimedia.org/wikipedia/commons/3/3c/%D0%94%D0%BE%D0%B1%D1%80%D1%8F%D0%BA.jpg',
    ...
    'ulytau': '1518098268026-4e89f1a2cd8e',
    'west-kazakhstan': '1500382017468-9049fed747ef',
    'kz-geo-general': '1518098268026-4e89f1a2cd8e',
    'kz-license-plates': '1449844908441-8829872d2607',
    'world-flags': '1532375810709-75b1da00537c',
    'world-facts': '1521295121783-8a321d551ad2',
    'kz-natural-wonders': '1528660493888-ab6f4761e036',
    'world-capitals': '1449844908441-8829872d2607',
    'kz-cuisine': '1555939594-58d7cb561ad1',
    'kz-history': '1542601098-8fc114e148e2',
    'kz-sports': '1461896836934-ffe607ba8211',
    'space-exploration': '1518770660439-4636190af475',
    'tech-it': '1518770660439-4636190af475',
    'kz-culture-mixed': '1532375810709-75b1da00537c',
  };
  const photoId = imageMap[id] || '1550989460-0adf9ea622e2';
...
export default function App() {
  ...
  const achievements: Achievement[] = [
    { id: 'first_quiz', title: t[currentLanguage].achievementsList.first_quiz.title, description: t[currentLanguage].achievementsList.first_quiz.desc, icon: <Zap size={20} />, unlocked: userStats.quizzesCompleted > 0, xpReward: 100 },
    { id: 'perfect_score', title: t[currentLanguage].achievementsList.perfect_score.title, description: t[currentLanguage].achievementsList.perfect_score.desc, icon: <Star size={20} />, unlocked: userStats.perfectScores > 0, xpReward: 250 },
    { id: 'level_5', title: t[currentLanguage].achievementsList.level_5.title, description: t[currentLanguage].achievementsList.level_5.desc, icon: <Compass size={20} />, unlocked: userStats.level >= 5, xpReward: 500 },
    { id: 'all_regions', title: t[currentLanguage].achievementsList.all_regions.title, description: t[currentLanguage].achievementsList.all_regions.desc, icon: <MapIcon size={20} />, unlocked: userStats.unlockedAchievements.includes('all_regions'), xpReward: 300 },
    { id: 'streak_3', title: t[currentLanguage].achievementsList.streak_3.title, description: t[currentLanguage].achievementsList.streak_3.desc, icon: <Flame size={20} />, unlocked: userStats.streak >= 3, xpReward: 400 },
  ];
  ...
  const chartData = useMemo(() => {
    return regionsData
      .map(r => ({
        name: r.name[currentLanguage].replace(' область', '').replace(' (город)', '').replace(' облысы', '').replace(' (қала)', ''),
        population: parseInt(r.population.replace(/\s/g, ''), 10),
        fullName: r.name[currentLanguage]
      }))
  ...
  const filteredRegions = useMemo(() => {
    return regionsData.filter(r => 
      r.name[currentLanguage].toLowerCase().includes(searchQuery.toLowerCase()) ||
      r.capital[currentLanguage].toLowerCase().includes(searchQuery.toLowerCase())
    );
  ...
  const regionMapping: Record<string, string[]> = {
    'Almaty': ['almaty', 'jetisu'],
    'Almaty (Alma-Ata)': ['almaty-city'],
    'Aqmola': ['akmola'],
    'Aqtöbe': ['aktobe'],
    'Atyrau': ['atyrau'],
    'East Kazakhstan': ['east-kazakhstan', 'abai'],
    'Zhambyl': ['jambyl'],
    'Qaraghandy': ['karaganda', 'ulytau'],
    'Qostanay': ['kostanay'],
    'Qyzylorda': ['kyzylorda'],
  ...
  const handleMapRegionClick = (regionName: string) => {
    ...
    const regionIds = regionMapping[regionName];
    // If multiple regions map to this shape, we just pick the first one for now
    // or we could show a selection, but for simplicity we pick the first
    const regionId = regionIds ? regionIds[0] : null;
    ...
    if (!region) {
      region = regionsData.find(r => 
        r.name['ru'].toLowerCase().includes(regionName.toLowerCase()) || 
        regionName.toLowerCase().includes(r.name['ru'].toLowerCase()) ||
        r.id.toLowerCase().includes(regionName.toLowerCase())
      ) || null;
    }

    if (region) {
      console.log('Matched region:', region.name[currentLanguage]);
    ...
    <div className="space-y-12 pb-20">
      <section className="relative h-[60vh] flex items-center justify-center overflow-hidden rounded-3xl mt-4 mx-4 bg-zinc-900 text-white">
        <div className="absolute inset-0 opacity-40">
          <img 
            src={getImageUrl('almaty-city', 2000, 1200)}
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl">
          ...
          >
            ...
            >
              {t[currentLanguage].startExploring}
            </button>
            <button 
      ...
      <section className="px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
          <div>
            <h2 className="text-4xl font-bold tracking-tight mb-2">{t[currentLanguage].popularDestinations}</h2>
            <p className="text-zinc-500">{t[currentLanguage].popularDesc}</p>
          </div>
          <button 
            onClick={() => setCurrentView('regions')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {regionsData.slice(0, 3).map((region, idx) => (
            ...
            >
              <img 
                src={getImageUrl(region.id, 800, 1200)}
              ...
              <div className="absolute inset-0 p-10 flex flex-col justify-end">
                <div className="mb-4 overflow-hidden">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-bold text-white/70 uppercase tracking-[0.2em]">{region.capital[currentLanguage]}</span>
                  </div>
      ...
      <section className="px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row gap-12 items-center">
          <div className="md:w-1/2">
            <h2 className="text-3xl font-bold tracking-tight mb-6">{t[currentLanguage].demographics}</h2>
            <p className="text-zinc-500 mb-8">
              {t[currentLanguage].demographicsDesc}
            </p>
            <div className="space-y-4">
              {chartData.slice(0, 3).map((item, i) => (
                <div key={item.name} className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl border border-zinc-100">
                  ...
                  <span className="text-zinc-500 font-medium">{item.population.toLocaleString()} {t[currentLanguage].people}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} layout="vertical" margin={{ left: 20, right: 20 }}>
                ...
                <Tooltip 
                  ...
                  formatter={(value: number) => [value.toLocaleString() + ' {t[currentLanguage].people}', t[currentLanguage].population]}
                />
      ...
      <section className="px-6 max-w-7xl mx-auto bg-zinc-50 py-16 rounded-3xl">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold tracking-tight mb-4">{t[currentLanguage].testKnowledge}</h2>
          <p className="text-zinc-500 max-w-2xl mx-auto">{t[currentLanguage].testKnowledgeDesc}</p>
        </div>
          ...
          {quizzesData.slice(0, 4).map((quiz) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="font-bold mb-1">{quiz.title[currentLanguage]}</h3>
                <p className="text-xs text-zinc-500 mb-4">{quiz.description[currentLanguage]}</p>
                <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">
                  {quiz.questions.length} {t[currentLanguage].questionsCount}
                </div>
              </motion.div>
            );
          })}
        ...
        <div className="text-center mt-10">
          ...
          >
            {currentLanguage === 'ru' ? 'Все квизы' : 'Барлық викториналар'}
          </button>
        </div>
      </section>
    </div>
  ...
  const renderRegions = () => (
    <div className="px-6 max-w-7xl mx-auto py-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          <h1 className="text-4xl font-bold tracking-tight mb-2">{t[currentLanguage].regionsTitle}</h1>
          <p className="text-zinc-500">{t[currentLanguage].regionsDesc}</p>
        </div>
        <div className="relative">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" size={20} />
          <input 
            type="text" 
            placeholder={t[currentLanguage].searchPlaceholder} 
            value={searchQuery}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-12">
        <div className="lg:col-span-2 h-[600px]">
          <MapComponent 
            ...
            selectedRegionId={previewRegion?.id}
            ...
            {previewRegion ? (
              ...
              >
                <div className="h-48 bg-zinc-100 rounded-2xl overflow-hidden mb-6">
                  <img 
                    src={getImageUrl(previewRegion.id, 800, 600)}
                    alt={previewRegion.name[currentLanguage]}
                    className="w-full h-full object-cover"
                    referrerPolicy="no-referrer"
                  />
                </div>
                <h3 className="text-2xl font-bold mb-2">{previewRegion.name[currentLanguage]}</h3>
                <p className="text-zinc-500 text-sm mb-6 line-clamp-4">{previewRegion.description[currentLanguage]}</p>
                
                <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="bg-zinc-50 p-3 rounded-xl">
                    <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">{t[currentLanguage].center}</div>
                    <div className="text-sm font-bold">{previewRegion.capital[currentLanguage]}</div>
                  </div>
                  <div className="bg-zinc-50 p-3 rounded-xl">
                    <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">{t[currentLanguage].area}</div>
                ...
                <div className="mt-auto space-y-3">
                  ...
                  >
                    {t[currentLanguage].moreDetails} <ChevronRight size={18} />
                  </button>
                  <button 
              ...
              >
                <h3 className="font-bold text-xl mb-4">{t[currentLanguage].selectRegionOnMap}</h3>
                {regionsData.map(region => (
                  ...
                  >
                    <div>
                      <div className="font-bold">{region.name[currentLanguage]}</div>
                      <div className="text-xs text-zinc-400 uppercase tracking-widest">{region.capital[currentLanguage]}</div>
                    </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[240px]">
        {filteredRegions.map((region, index) => {
          ...
          return (
            ...
            >
              <img 
                src={getImageUrl(region.id, 1200, 800)}
                alt={region.name[currentLanguage]}
                ...
                <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2 py-0.5 bg-white/20 backdrop-blur-md rounded-full text-[10px] font-bold text-white uppercase tracking-wider">
                      {region.capital[currentLanguage]}
                    </span>
                  </div>
      ...
      {filteredRegions.length === 0 && (
        <div className="text-center py-20">
          <p className="text-zinc-500 text-lg">{t[currentLanguage].nothingFound}</p>
        </div>
      )}
    </div>
  );
    ...
    return (
      <div className="pb-20">
        <div className="relative h-[50vh] w-full">
          <img 
            src={getImageUrl(selectedRegion.id, 1920, 1080)}
            alt={selectedRegion.name[currentLanguage]}
            className="w-full h-full object-cover"
            referrerPolicy="no-referrer"
          />
          ...
          <div className="absolute bottom-0 left-0 w-full p-8 md:p-16">
            <div className="max-w-7xl mx-auto">
              ...
              >
                <ArrowLeft size={20} /> {t[currentLanguage].backToRegions}
              </button>
              <h1 className="text-4xl md:text-6xl font-bold text-white mb-4 tracking-tight">{selectedRegion.name[currentLanguage]}</h1>
              <div className="flex flex-wrap gap-6 text-white/80 font-medium">
                <div className="flex items-center gap-2">
                  <Landmark size={18} className="text-zinc-400" />
                  <span>{t[currentLanguage].center}: {selectedRegion.capital[currentLanguage]}</span>
                </div>
                <div className="flex items-center gap-2">
                  <MapIcon size={18} className="text-zinc-400" />
                  <span>{t[currentLanguage].area}: {selectedRegion.area}</span>
                </div>
                <div className="flex items-center gap-2">
        ...
        <div className="max-w-4xl mx-auto px-6 py-16">
          <div className="prose prose-zinc lg:prose-xl">
            <h2 className="text-3xl font-bold mb-6">{t[currentLanguage].aboutRegion}</h2>
            <p className="text-zinc-600 leading-relaxed text-lg mb-12">
              {selectedRegion.description[currentLanguage]}
            </p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
              <div className="p-8 bg-zinc-50 rounded-3xl border border-zinc-100">
                <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
                  <Info size={20} /> {t[currentLanguage].geography}
                </h3>
              ...
              <div className="p-8 bg-zinc-50 rounded-3xl border border-zinc-100">
                <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
                  <Star size={20} /> {t[currentLanguage].interestingFact}
                </h3>
                <p className="text-zinc-600 text-sm">
  ...
  const renderProfile = () => {
    ...
    return (
      <div className="px-6 max-w-4xl mx-auto py-12">
          ...
          <div className="relative z-10">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h1 className="text-3xl font-bold mb-1">{currentUser?.displayName || t[currentLanguage].explorer}</h1>
                  <p className="text-zinc-400">{currentUser?.email || t[currentLanguage].guestMode}</p>
                </div>
              </div>
              ...
              {currentUser ? (
                ...
                >
                  <LogOut size={18} /> {t[currentLanguage].logout}
                </button>
              ) : (
                <button 
                  onClick={handleLogin}
                ...
                >
                  <LogIn size={18} /> {t[currentLanguage].loginGoogle}
                </button>
              )}
            </div>
            ...
            <div className="space-y-2 mb-8">
              <div className="flex justify-between text-sm font-bold uppercase tracking-widest">
                <span>{t[currentLanguage].experience}: {userStats.xp} XP</span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                <div className="text-zinc-400 text-xs font-bold uppercase tracking-widest mb-1">{t[currentLanguage].quizzes}</div>
                <div className="text-2xl font-bold">{userStats.quizzesCompleted}</div>
              </div>
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                <div className="text-zinc-400 text-xs font-bold uppercase tracking-widest mb-1">{t[currentLanguage].records}</div>
                <div className="text-2xl font-bold">{userStats.perfectScores}</div>
              </div>
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                <div className="text-zinc-400 text-xs font-bold uppercase tracking-widest mb-1">{t[currentLanguage].streak}</div>
                <div className="text-2xl font-bold">{userStats.streak}</div>
              </div>
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                <div className="text-zinc-400 text-xs font-bold uppercase tracking-widest mb-1">{t[currentLanguage].rank}</div>
        ...
        <h2 className="text-2xl font-bold mb-8">{t[currentLanguage].achievements}</h2>
              ...
              <div>
                ...
                {achievement.unlocked && (
                  <div className="mt-2 text-xs font-bold text-emerald-600 uppercase tracking-widest flex items-center gap-1">
                    <CheckCircle2 size={12} /> {t[currentLanguage].unlocked} (+{achievement.xpReward} XP)
                  </div>
                )}
              </div>
            </div>
  ...
  const renderQuizzes = () => (
    <div className="px-6 max-w-7xl mx-auto py-12">
      <div className="mb-12">
        <h1 className="text-4xl font-bold tracking-tight mb-2">{t[currentLanguage].interactiveQuizzes}</h1>
        <p className="text-zinc-500">{t[currentLanguage].testKnowledge} и узнайте новые факты о Казахстане.</p>
      </div>
          ...
          return (
            ...
            >
              <div className="absolute inset-0 bg-zinc-100">
                 <img 
                  src={getImageUrl(quiz.id, 800, 1200)}
                  alt={quiz.title[currentLanguage]}
              ...
              <div className="absolute inset-0 p-10 flex flex-col justify-end">
                ...
                <h3 className="text-3xl font-bold text-white mb-4 leading-tight">{quiz.title[currentLanguage]}</h3>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-white/50 text-xs font-bold uppercase tracking-widest">
                    <Target size={16} /> {quiz.questions.length} {t[currentLanguage].questionsCount}
                  </div>
    ...
    if (quizState.isFinished) {
      ...
      return (
        <div className="max-w-2xl mx-auto px-6 py-20 text-center">
          ...
          <h1 className="text-4xl font-bold mb-4">{currentLanguage === 'ru' ? 'Квиз завершен!' : 'Викторина аяқталды!'}</h1>
          <p className="text-xl text-zinc-500 mb-12">
            {t[currentLanguage].yourScore}: <span className="text-black font-bold">{quizState.score}</span> {t[currentLanguage].of} {selectedQuiz.questions.length} ({percentage}%)
          </p>
          ...
          <div className="grid grid-cols-1 gap-4 mb-12">
            ...
            >
              {t[currentLanguage].backToList} {currentLanguage === 'ru' ? 'квизов' : 'викториналар'}
            </button>
          </div>
        </div>
      );
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm font-bold text-zinc-400 uppercase tracking-widest">
              {t[currentLanguage].question} {quizState.currentQuestionIndex + 1} {t[currentLanguage].of} {selectedQuiz.questions.length}
            </span>
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {currentQuestion.type === 'multiple-choice' && !quizState.showResult && (
              ...
              >{currentLanguage === 'ru' ? 'Подтвердить выбор' : 'Таңдауды растау'}</button>
            )}
            ...
            {quizState.showResult && (
              ...
              >
                ...
                <p className="text-zinc-300 mb-6">{currentQuestion.explanation[currentLanguage]}</p>
                <button 
                  onClick={nextQuestion}
                  className="w-full py-4 bg-white text-black rounded-xl font-bold hover:bg-zinc-200 transition-colors"
                >
                  {quizState.currentQuestionIndex === selectedQuiz.questions.length - 1 ? '{t[currentLanguage].finish}' : t[currentLanguage].next}
                </button>
              </motion.div>
            )}
          </motion.div>
    ...
    <div className="min-h-screen bg-white text-zinc-900 font-sans selection:bg-zinc-900 selection:text-white">
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-zinc-100">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            >
              {t[currentLanguage].regionsTitle.split(" ")[0]}
            </button>
            <button 
              onClick={() => setCurrentView('quizzes')}
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 leading-none">
                  {currentUser ? currentUser.displayName?.split(' ')[0] : t[currentLanguage].rank}
                </span>
            ...
            >
              <div className="px-6 py-8 flex flex-col gap-6">
                ...
                <button className="w-full py-4 bg-zinc-900 text-white rounded-2xl font-bold">
                  {t[currentLanguage].loginGoogle.split(" ")[0]}
                </button>
              </div>
            </motion.div>
          )}
      ...
      <footer className="bg-zinc-50 border-t border-zinc-200 py-20 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          <div className="col-span-1 md:col-span-2">
            ...
            <p className="text-zinc-500 max-w-sm mb-8">
              {currentLanguage === 'ru' ? 'Образовательная платформа для изучения географии, истории и культуры Казахстана в интерактивном формате.' : 'Қазақстанның географиясын, тарихын және мәдениетін интерактивті форматта үйренуге арналған білім беру платформасы.'}
            </p>
          ...
          <div>
            <h4 className="font-bold mb-6 uppercase tracking-widest text-xs text-zinc-400">{currentLanguage === 'ru' ? 'Навигация' : 'Навигация'}</h4>
            <ul className="space-y-4 font-medium">
              <li><button onClick={() => setCurrentView('home')} className="hover:text-zinc-500 transition-colors">{t[currentLanguage].home}</button></li>
              <li><button onClick={() => setCurrentView('regions')} className="hover:text-zinc-500 transition-colors">{t[currentLanguage].regionsTitle.split(" ")[0]}</button></li>
              <li><button onClick={() => setCurrentView('quizzes')} className="hover:text-zinc-500 transition-colors">{t[currentLanguage].quizzes}</button></li>
            </ul>
          </div>
          <div>
            <h4 className="font-bold mb-6 uppercase tracking-widest text-xs text-zinc-400">{currentLanguage === 'ru' ? 'О проекте' : 'Жоба туралы'}</h4>
            <ul className="space-y-4 font-medium">
              <li><button className="hover:text-zinc-500 transition-colors">{currentLanguage === 'ru' ? 'О нас' : 'Біз туралы'}</button></li>
              <li><button className="hover:text-zinc-500 transition-colors">{currentLanguage === 'ru' ? 'Контакты' : 'Байланыстар'}</button></li>
              <li><button className="hover:text-zinc-500 transition-colors">{currentLanguage === 'ru' ? 'Политика конфиденциальности' : 'Құпиялылық саясаты'}</button></li>
            </ul>
          </div>
        </div>
        <div className="max-w-7xl mx-auto mt-20 pt-8 border-t border-zinc-200 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-zinc-400">
          <p>© 2026 QazaqGeo Learn. {currentLanguage === 'ru' ? 'Все права защищены.' : 'Барлық құқықтар қорғалған.'}</p>
          <p>{currentLanguage === 'ru' ? 'Сделано с любовью к Казахстану' : 'Қазақстанға деген сүйіспеншілікпен жасалған'} 🇰🇿</p>
        </div>
      </footer>
    </div>
  );
